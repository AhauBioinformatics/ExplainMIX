...
def get_pred(adj_mats,num_relations,tape,pred,head,tail,top_k):
    ...
    for i in range(num_relations):
        ...
        for a in k_i:
            ...
            k = adj_mat_i.indices[a[0], 2]

            p_logic = weight_calulate.logic_weight(head, tail, pred, k.numpy(),'k_i')
            p_rele = weight_calulate.relevent_weight(head, adj_mat_i.indices[a[0], 2])
            weight = p_logic * (1+p_rele)
            score = weight*score
        ...
        for b in k_j:
            k = adj_mat_i.indices[b[0], 2]
            p_logic = weight_calulate.logic_weight(head, tail, pred, k.numpy(), 'k_j')
            p_rele = weight_calulate.relevent_weight(tail, k)

            weight = p_logic * (1 + p_rele)
            score = weight * abs(score)
...
if __name__ == '__main__':
    ...
    model.load_weights(os.path.join('..', 'data', 'weights', 'drug_celline', 'drug_celline' + '_' + RULE + '.weights.h5'))
