...
class RGCN_Layer(tf.keras.layers.Layer):
    ...
    def call(self,inputs):
        ...
        for i in range(self.num_relations):
            ...
            head_idx = tf.cast(head_idx, dtype=tf.int32)
            tail_idx = tf.cast(tail_idx, dtype=tf.int32)
...
class DistMult(tf.keras.layers.Layer):
    ...
    def call(self,inputs):
        ...
        rel_idx = tf.cast(rel_idx, dtype=tf.int32)
...
if __name__ == '__main__':
    ...
    model.save_weights(
        os.path.join('..', 'data', 'weights', 'drug_celline', 'drug_celline' + '_' + RULE + '.weights.h5'))